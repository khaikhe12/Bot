...
def testar_api():
    ...
    try:
        data = {"mensagem": "oi", "user_id": "5511999999999"}
    ...
    try:
        data = {"mensagem": "João Silva", "user_id": "5511999999999"}
        response = requests.post(f"{BASE_URL}/mensagem", json=data)
        print(f"✅ Status: {response.status_code}")
        print(f"📄 Resposta: {response.json()}\n")
    except Exception as e:
    ...
    try:
        data = {"mensagem": "1", "user_id": "5511999999999"}
        response = requests.post(f"{BASE_URL}/mensagem", json=data)
        print(f"✅ Status: {response.status_code}")
        print(f"📄 Resposta: {response.json()}\n")
    except Exception as e:
...
def simular_conversa_completa():
    ...
    for i, mensagem in enumerate(mensagens, 1):
        ...
        try:
            data = {"mensagem": mensagem, "user_id": numero_cliente}
            response = requests.post(f"{BASE_URL}/mensagem", json=data)
